  ...
  const handleNavigation = (e: React.MouseEvent<HTMLAnchorElement>, target: string) => {
    ...
    if (target.startsWith('/')) {
      ...
      if (location.pathname === '/') {
        ...
        // We are elsewhere, go home then scroll (handled by HomePage useEffect)
        ...
        <div className="container mx-auto px-6 md:px-10 py-16 md:py-20 flex flex-col lg:flex-row items-center justify-between gap-10">
          <div className="max-w-3xl text-center lg:text-left">
            <h3 className="text-3xl md:text-4xl font-black text-white mb-4 leading-tight uppercase">
              ТЕХНОЛОГИЧЕН ПАРТНЬОР ЗА <br className="hidden md:block"/>
              <span className="text-geo-yellow">КРИТИЧНА ИНФРАСТРУКТУРА</span>
            </h3>
            <p className="text-gray-400 text-lg font-light">
              От лабораторния анализ до изпълнението на обекта. Нека обсъдим вашите технически предизвикателства и цели за декарбонизация.
            </p>
          </div>
          <div className="w-full lg:w-auto flex justify-center">
             <a href="/request-inspection" onClick={(e) => handleNavigation(e, '/request-inspection')} className="w-full sm:w-auto">
              <Button variant="primary" className="w-full sm:w-auto py-5 px-10 text-sm font-bold shadow-[0_0_20px_rgba(255,204,0,0.2)] uppercase">
                СВЪРЖЕТЕ СЕ С ЕКИПА <ArrowRight className="ml-2 w-5 h-5" />
              </Button>
            </a>
          </div>
        </div>
      ...
      <div className="w-full bg-[#0a0a0a]">
        <div className="container mx-auto px-6 md:px-10 pt-20 pb-10">
          ...
          <div className="col-span-1">
            <div className="flex items-center mb-6 cursor-pointer" onClick={(e) => handleNavigation(e as any, '/')}>
              <img 
                 src="/logoGEONYX.png" 
                 alt="GEONYX" 
            ...
            <p className="text-gray-400 text-sm font-normal leading-relaxed mb-8">
              Разработваме високотехнологични геополимерни матрици за критична инфраструктура и тежка индустрия. Инженеринг отвъд границите на бетона.
            </p>
          ...
          <div>
            <h5 className="text-white font-bold uppercase tracking-widest text-xs mb-8 flex items-center gap-2">
              <span className="w-2 h-2 bg-geo-yellow"></span> ИНЖЕНЕРНИ СИСТЕМИ
            </h5>
            <ul className="space-y-4">
              <li>
                <a href="/armor" onClick={(e) => handleNavigation(e, '/armor')} className="text-gray-400 hover:text-geo-yellow transition-colors text-sm font-normal uppercase tracking-wide">
                  GEONYX ARMOR (Механика)
                </a>
              </li>
              <li>
                <a href="/chem" onClick={(e) => handleNavigation(e, '/chem')} className="text-gray-400 hover:text-geo-yellow transition-colors text-sm font-normal uppercase tracking-wide">
                  GEONYX CHEM (Химия)
                </a>
              </li>
              <li>
                <a href="/therm" onClick={(e) => handleNavigation(e, '/therm')} className="text-gray-400 hover:text-geo-yellow transition-colors text-sm font-normal uppercase tracking-wide">
                  GEONYX THERM (Термошок)
                </a>
              </li>
              <li>
                <a href="/hydro" onClick={(e) => handleNavigation(e, '/hydro')} className="text-gray-400 hover:text-geo-yellow transition-colors text-sm font-normal uppercase tracking-wide">
                  GEONYX HYDRO (Хидротехника)
                </a>
              </li>
            </ul>
          </div>
          ...
          <div>
            <h5 className="text-white font-bold uppercase tracking-widest text-xs mb-8 flex items-center gap-2">
              <span className="w-2 h-2 bg-geo-yellow"></span> ИНЖЕНЕРЕН ПОРТАЛ
            </h5>
            <ul className="space-y-4">
              <li>
                <a href="/technical-maps" onClick={(e) => handleNavigation(e, '/technical-maps')} className="text-gray-400 hover:text-white transition-colors text-sm font-normal uppercase tracking-wide">
                  Технически карти (TDS / MSDS)
                </a>
              </li>
              <li>
                <a href="/cad-bim" onClick={(e) => handleNavigation(e, '/cad-bim')} className="text-gray-400 hover:text-white transition-colors text-sm font-normal uppercase tracking-wide">
                  CAD & BIM Библиотека
                </a>
              </li>
              <li>
                <a href="/certificates" onClick={(e) => handleNavigation(e, '/certificates')} className="text-gray-400 hover:text-white transition-colors text-sm font-normal uppercase tracking-wide">
                  Сертификати & Съответствие
                </a>
              </li>
              <li>
                <a href="/references" onClick={(e) => handleNavigation(e, '/references')} className="text-gray-400 hover:text-white transition-colors text-sm font-normal uppercase tracking-wide">
                  Оперативен архив (Case Studies)
                </a>
              </li>
              <li>
                <a href="/hseq" onClick={(e) => handleNavigation(e, '/hseq')} className="text-gray-400 hover:text-white transition-colors text-sm font-normal uppercase tracking-wide">
                  Корпоративен стандарт (HSEQ)
                </a>
              </li>
            </ul>
          </div>
          ...
          <div>
            <h5 className="text-white font-bold uppercase tracking-widest text-xs mb-8 flex items-center gap-2">
              <span className="w-2 h-2 bg-geo-yellow"></span> ГЛОБАЛНИ КОНТАКТИ
            </h5>
            <ul className="space-y-6">
              <li className="flex items-start group">
                <MapPin className="w-5 h-5 text-gray-500 group-hover:text-geo-yellow mr-4 mt-0.5 transition-colors" />
                <span className="text-gray-400 text-sm font-normal">гр. Габрово,<br/>ул. „Юрий Венелин“ 18, ет. 2<br/>5300 Габрово, България</span>
              </li>
          ...
          {/* COLUMN 5: TECHNICAL BULLETIN */}
          <div>
            <h5 className="text-white font-bold uppercase tracking-widest text-xs mb-8 flex items-center gap-2">
              <span className="w-2 h-2 bg-geo-yellow"></span> МРЕЖИ И АКТУАЛИЗАЦИИ
            </h5>
            ...
            <div className="border-t border-gray-800 pt-8">
              <label className="block text-[10px] uppercase font-bold text-gray-500 tracking-wider mb-3">
                АБОНАМЕНТ ЗА ТЕХНИЧЕСКИ БЮЛЕТИН:
              </label>
              ...
              <p className="text-[10px] text-gray-600 font-normal leading-tight">
                  Получавайте известия само при обновяване на технически спецификации (TDS), нови EPD декларации и нормативни промени.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-[#222] pt-8 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="text-gray-600 text-xs font-bold uppercase tracking-wider">
              © 2026 GEONYX GROUP. Всички права запазени.
          </div>

          {/* Center: Legal Links */}
          <div className="flex flex-wrap justify-center gap-6 md:gap-8">
              <a href="/privacy-policy" onClick={(e) => handleNavigation(e, '/privacy-policy')} className="text-gray-500 text-xs hover:text-white transition-colors uppercase tracking-wide font-bold">
              ...
              <a href="/terms-conditions" onClick={(e) => handleNavigation(e, '/terms-conditions')} className="text-gray-500 text-xs hover:text-white transition-colors uppercase tracking-wide font-bold">
                  Общи условия
              </a>
              <a href="/hseq" onClick={(e) => handleNavigation(e, '/hseq')} className="text-gray-500 text-xs hover:text-white transition-colors uppercase tracking-wide font-bold">
                  ESG & HSEQ Декларация
              </a>
          </div>

          {/* Right: Trust Badges (Adjusted Size) */}
