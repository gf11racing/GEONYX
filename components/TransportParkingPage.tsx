...
import {
  Truck, Car, Plane, ShieldCheck,
  AlertTriangle, FileText, ArrowRight, Download,
  HelpCircle, CheckCircle, Copy, Construction,
  Navigation, Layers, Wrench, Check, CloudRain, Signpost, Activity, BatteryCharging
} from 'lucide-react';
...
export const TransportParkingPage: React.FC = () => {
  usePageMeta('transportparking');
  const { t } = useTranslation('transportparking');
  const { to } = useLang();
  ...
  const appIcons = [Car, Layers, Navigation, Truck, Plane, Construction, Wrench, ShieldCheck];
  ...
  const blockImages = [
    '/GEONYX-INTERMEDIATE-LEVELS.webp',
    '/GEONYX-OPEN-AND-COVERED=PARKING-LOTS.webp',
    '/GEONYX-RAMPS-AND-INCLINED-APPROACHES.webp',
    '/GEONYX-TIR-ZONES.webp',
    '/GEONYX-MARKING-AND-VISUAL-DESIGN.webp',
    '/GEONYX-EV-ZONES-AND-FAST-CHARGING.webp',
  ];
  const blockIcons = [Car, CloudRain, Activity, Truck, Signpost, BatteryCharging];
  const blocks = ['b1', 'b2', 'b3', 'b4', 'b5', 'b6'] as const;
                  ...
                  <table className="w-full min-w-[900px] border-collapse text-left">
                      <thead>
                          <tr className="bg-black">
                              ...
                              <th className="p-4 text-gray-500 font-bold uppercase tracking-wider text-xs border-b border-[#333] w-1/4">{t('roi.col_bitumen')}</th>
                          </tr>
                      </thead>
